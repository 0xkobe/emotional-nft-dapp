...
const Tooltip: FunctionComponent<IProps> = ({ tooltip, tooltipClassName, children, className }: IProps) => {
  const [ref, isHover] = useHover<HTMLDivElement>()

  return (
    <div className={classNames(className, "relative z-30 inline-flex")}>
      ...
      {isHover && (
        <div className="absolute left-2/4 w-0 h-full">
          <div className="relative h-full">
            ...
            <div className="absolute z-20 -ml-1 top-full w-2 h-2 bg-white border-l border-t border-gray-200 transform rotate-45">
            </div>
          </div>
        </div>
      )}
